...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $psuWattage = getWattage('psuList', 'psu_id', $psu_id);
    ...
    $totalWattage = $psuWattage + $cpuWattage + $gpuWattage + $moboWattage;
}
        ...
        <form method="POST" action="">
            <div class="mb-3">
                ...
                <select name="psu" class="form-select">
                    <option value="">Seleccione una PSU</option>
                    <?php foreach ($psuOptions as $option) : ?>
                        <option value="<?php echo $option['psu_id']; ?>" <?php echo isset($psu_id) && $psu_id == $option['psu_id'] ? 'selected' : ''; ?>>
                            <?php echo htmlspecialchars($option['psuName']); ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
